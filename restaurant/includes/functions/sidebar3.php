<?php
/*==================================================
  run PHP in widget - run shortcode in widget
  ==================================================*/
...
/*========================================================================
/* Display total number of post for specific category.   two  methods echo count_cat_post('travel') */
/*=========================================================================*/
...
//method 2    echo count_cat_post('travel')  - slug
...
/*============================================================================*/
// Shortcode to count product category - not count parent if there is children
/*============================================================================*/
...
//====================================================================================================
// by category - sidebar to display post of categories  - echo do_shortcode("[categoryposts_sidebar]")
//=====================================================================================================*/
/*5 posts per page */
...
/*=======================================*/
// create slider short code for product
/*=======================================*/
function wpb_productcategory_sidebar () {	
	
	
	     		<?php
      		$dashboardIcon = [ "Thai Food",  "Bars and Beverages", "Chinese Food", "Dim Sum", "Cafe", "Vietnamese", "Dumplings", "Pasta"];
					$dashboardLength = count($dashboardIcon);
        	$i = 0;
					while ($i < $dashboardLength) {	
				 	$plural = $dashboardIcon[$i];
			...
			// The Loop
						if ( $the_query->have_posts() ) {
							$query = new WP_Query( array(
								...
								<div class="<?php echo $singular; ?>">
									<?php while ( $the_query->have_posts() ) {
										$the_query->the_post();
												if ( has_post_thumbnail() ) { ?>
												<div class="border-top py-3 d-flex align-items-center"><a href="<?php the_permalink() ;?>" rel="bookmark"><?php the_post_thumbnail( $post->ID, array( 35, 35),  array( 'class' => 'rounded shadow-sm nolazyload' ) ); ?><span class="pl-4"><?php echo wp_trim_words( get_the_title(), 10 , '   .  .  .' ); ?></span></a></div> 		
													<?php		} else {  ?>
			<!--									// if no featured image is found-->
												<hr>
												<div class="py-2"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php echo  wp_trim_words( get_the_title(), 10 , '   .  .  .' ); ?></a></div> 
												}
										} ?>
								</div>
								<?php  $i++; } else { ?>
...
// the query 
			...
			if ( $the_query->have_posts() ) {
				...
				$total_count = esc_html( $query->post_count );
					$string .= '<h6 class="pt-3 text-center"><a href="#">'.$plural.  '  <span class="badge badge-secondary">' .	$total_count .'</span></a></h6>';
					$string .= '<div class='.$singular.'>';
					while ( $the_query->have_posts() ) {
							$the_query->the_post();
									if ( has_post_thumbnail() ) {
									$string .= '<li>';
									$string .=  '<div class="border-top py-3 d-flex align-items-center"><a href="' . get_the_permalink() .'" rel="bookmark">'.get_the_post_thumbnail( $post->ID, array( 35, 35),  array( 'class' => 'rounded shadow-sm nolazyload' ) ). '<span class="pl-4"><a href="' . get_the_permalink() .'" rel="bookmark">' .  wp_trim_words( get_the_title(), 10 , '   .  .  .' ) . '</span></a></div></li>'; 		
									} else { 
									// if no featured image is found
									$string .= '<hr>';
									$string .= '<li class="py-2"><a href="' . get_the_permalink() .'" rel="bookmark">' . wp_trim_words( get_the_title(), 10 , '   .  .  .' ) . '</a></li>'; 
